  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        {[{ key: "", label: "全部" }, ...CATEGORIES.map((c) => ({ key: c, label: CAT_LABELS[c] }))].map(
          (f) => (
            <button
              key={f.key}
          ...
          <thead>
            <tr className="border-b border-surface-800 bg-surface-900/50">
              <th className={thClass} onClick={() => toggleSort("title")}>
                <span className="flex items-center gap-1.5">标题 <SortIcon field="title" sort={sort} /></span>
              </th>
              <th className={thClass + " w-28"} onClick={() => toggleSort("category")}>
                <span className="flex items-center gap-1.5">分类 <SortIcon field="category" sort={sort} /></span>
              </th>
              ...
              <th className={thClass + " w-24"} onClick={() => toggleSort("author")}>
                <span className="flex items-center gap-1.5">作者/地区 <SortIcon field="author" sort={sort} /></span>
              </th>
              <th className={thClass + " w-28"} onClick={() => toggleSort("date")}>
                <span className="flex items-center gap-1.5">日期 <SortIcon field="date" sort={sort} /></span>
              </th>
            ...
            {filtered.map((l) => (
              <tr key={l.id} className="hover:bg-surface-800/20 transition-colors duration-150">
                ...
                <td className="px-5 py-3.5">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium ${CAT_COLORS[l.category] || "bg-surface-800 text-surface-400"}`}>
