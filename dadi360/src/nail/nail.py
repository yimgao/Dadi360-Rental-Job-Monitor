...
logger.add(
    os.path.join(LOGS_DIR, "nail_jobs_info.log"),
    rotation="10 MB",
    retention="30 days",
    level="INFO",
...
logger.add(
    os.path.join(LOGS_DIR, "nail_jobs_error.log"),
    rotation="5 MB",
    retention="60 days",
    level="ERROR",
    format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}",
...
_sent_nail_ids = load_sent_ids(SENT_IDS_FILE)
...
def parse_html_for_nail_jobs(html_content: str, base_url: str, search_terms: list) -> list[dict]:
    """
    纯函数：解析HTML内容，提取符合条件的美甲招聘信息。
    """
...
def format_email_body(jobs: list[dict], search_terms: list) -> tuple[str, str]:
    ...
    subject = f"【美甲招聘】{search_terms_str}等招聘信息通知"
    
    # 生成统计总结
    summary = summarize_jobs_by_date(jobs)
    
    body = f"你好！\n\n我们发现以下新的美甲招聘信息（匹配关键词：{', '.join(search_terms)}）：\n"
    body += summary  # 添加统计总结
...
def scrape_and_notify_nail_jobs(config: dict, current_sent_ids: set) -> set:
    """
    主协调函数：执行美甲招聘信息抓取、筛选和通知的整个流程。
    """
    try:
        logger.info(f"--- 美甲招聘监控任务开始: {time.strftime('%Y-%m-%d %H:%M:%S')} ---")
        all_raw_jobs = []
        ...
        for page_num, page_url in enumerate(nail_urls, 1):
            try:
                logger.info(f"正在抓取第 {page_num} 页: {page_url}")
                html_content = fetch_html(page_url, config["HEADERS"])
                if html_content:
                    # 使用美甲招聘关键词
                    nail_keywords = config.get("nail_jobs", {}).get("keywords", ["美甲", "指甲", "甲店", "小工", "大工"])
                    jobs_on_page = parse_html_for_nail_jobs(
                        html_content, page_url, nail_keywords
                    )
        ...
        if new_jobs:
            ...
            try:
                ...
                subject, body = format_email_body(new_jobs, nail_keywords)
            ...
            try:
                nail_keywords = config.get("nail_jobs", {}).get("keywords", ["美甲", "指甲", "甲店", "小工", "大工"])
                search_terms_str = "、".join(nail_keywords[:3])
                logger.info(f"此次任务未发现新的 '{search_terms_str}等' 招聘信息。")
            except Exception as e:
                logger.error(f"处理无新信息时出错: {e}")
        
        logger.info(f"--- 美甲招聘监控任务结束: {time.strftime('%Y-%m-%d %H:%M:%S')} ---\n")
        return updated_sent_ids
        
    except Exception as e:
        logger.error(f"美甲招聘监控任务发生严重错误: {e}")
...
def scheduled_task():
    """调度任务函数"""
    global _sent_nail_ids
    try:
        _sent_nail_ids = scrape_and_notify_nail_jobs(CONFIG, _sent_nail_ids)
        save_sent_ids(SENT_IDS_FILE, _sent_nail_ids)
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        nail_config = CONFIG.get("nail_jobs", {})
        keywords = nail_config.get("keywords", ["美甲", "指甲", "甲店", "小工", "大工"])
        keywords_str = "、".join(keywords[:5])
        interval_seconds = nail_config.get("send_interval_seconds", 172800)  # 默认2天
