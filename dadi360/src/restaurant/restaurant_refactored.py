...
class RestaurantJobScraper(BaseScraper):
    """餐厅招聘信息抓取器"""
    
    def __init__(self, config: dict):
        """初始化餐厅招聘抓取器"""
        sent_ids_file = os.path.join(os.path.dirname(__file__), "sent_restaurant_ids.json")
        super().__init__(config, "restaurant_jobs", sent_ids_file)
    ...
    def get_search_keywords(self) -> list[str]:
        """获取餐厅招聘搜索关键词"""
        return self.config.get("restaurant_jobs", {}).get("keywords", [
            "餐厅", "餐馆", "厨师", "企台", "收银", "服务员", "打杂", "油锅", "寿司", 
            "铁板", "外卖", "炒锅", "帮炒", "甜不辣", "teriyaki", "sushi", "企枱", 
            "前台", "打包", "炉尾", "busboy", "runner", "server", "点心", "推车", 
            "烧烤", "餐车", "奶茶", "沙拉", "卤味", "炸串", "冰淇淋", "面包", "西点", 
            "小笼包", "麻辣香锅"
        ])
    
    def parse_html_for_jobs(self, html_content: str, base_url: str, search_terms: list[str]) -> list[dict]:
        """解析HTML内容，提取符合条件的餐厅招聘信息"""
        ...
        for row in topic_rows:
            ...
            if title_link:
                ...
                if any(search_term in title for search_term in search_terms):
                    ...
                    logger.info(f"找到匹配餐厅招聘信息: {title}")
...
def main():
    ...
    try:
        ...
        # 创建餐厅招聘抓取器
        scraper = RestaurantJobScraper(config)
        
        logger.info("餐厅招聘监控脚本已启动。")
        keywords = scraper.get_search_keywords()
        keywords_str = "、".join(keywords[:5])
        interval_seconds = config.get("restaurant_jobs", {}).get("send_interval_seconds", 172800)  # 默认2天
